...
- **Delivery tag Leader 解析**: delivery tag topology 优先使用节点本地 Slot runtime，再使用本节点已应用的 controller-leader runtime view；缓存不能覆盖所需 Slot 时只异步触发一个合并后的严格 Controller 刷新，当前投递立即进入重试，不能在共享投递重试轮上等待 RPC。严格刷新限制在 500ms 内，成功快照短暂复用，失败也按短退避缓存，避免按消息数放大 Controller 压力。非本地但全局有效的 Slot 不能按 `slot not found` 处理；runtime view 必须有 quorum 且 `ObservedConfigEpoch` 不落后于 assignment，否则应让投递解析重试。
