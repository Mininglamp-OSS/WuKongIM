...
- **Delivery tag Leader 解析**: delivery tag topology 优先使用节点本地 Slot runtime，再使用本节点已应用且未过期的 controller-leader runtime view；等价 runtime view 的 full-sync 更新必须续写 `LastReportAt` 并同步到 follower，避免稳定拓扑被误判过期。缓存不能覆盖所需 Slot 时，首个调用最多等待 500ms 做严格 Controller 刷新，并发调用共享同一刷新。共享刷新与 owner 的取消信号隔离；成功快照缓存 5s（覆盖默认 resolve 重试预算），失败或快照仍不完整时按 500ms 短退避，避免按消息数串行放大 Controller 压力。严格结果只用有效 leader 覆盖已有观测，不丢弃本地缓存对省略或暂时无有效 leader 的 Slot 解析。非本地但全局有效的 Slot 不能按 `slot not found` 处理；runtime view 必须有 leader、quorum、对应 assignment，且 `ObservedConfigEpoch` 不落后于 assignment，否则应让投递解析重试。若 assignment 严格刷新推进了本地 HashSlotTable 版本，必须按新表重新计算 UID 对应的 Slot，不能给旧 Slot 引用标记新表版本。若 assignment 控制面读取失败但全部目标 Slot 已有本地可用 leader，则继续使用零 epoch topology，避免控制面同步窗口丢弃已提交消息；只有权威读取成功并确认 assignment 缺失时才拒绝。
