...
- **Delivery tag Leader 解析**: delivery tag topology 优先使用节点本地 Slot runtime，再使用本节点已应用的 controller-leader runtime view；只有缓存不能覆盖所需 Slot 时才严格刷新一次 Controller 视图，且该刷新必须限制在 500ms 内，不能阻塞共享的投递重试轮。非本地但全局有效的 Slot 不能按 `slot not found` 处理；runtime view 必须有 quorum 且 `ObservedConfigEpoch` 不落后于 assignment，否则应让投递解析重试。
