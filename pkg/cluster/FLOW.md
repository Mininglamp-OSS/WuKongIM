...
- **Delivery tag Leader 解析**: delivery tag topology 优先使用节点本地 Slot runtime，再使用本节点已应用的 controller-leader runtime view；缓存不能覆盖所需 Slot 时，首个调用最多等待 500ms 做严格 Controller 刷新，并发调用共享同一刷新。成功快照缓存 5s（覆盖默认 resolve 重试预算），失败或快照仍不完整时按 500ms 短退避，避免按消息数串行放大 Controller 压力。严格结果只覆盖其实际包含的 Slot，不丢弃本地缓存对其他 Slot 的有效解析。非本地但全局有效的 Slot 不能按 `slot not found` 处理；runtime view 必须有 quorum 且 `ObservedConfigEpoch` 不落后于 assignment，否则应让投递解析重试。
