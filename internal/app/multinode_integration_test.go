...
func TestDeliveryTagTopologyResolvesNonLocalSlotInFiveNodeCluster(t *testing.T) {
	...
	cachedViews, ok := materializer.Cluster().(interface {
		ListCachedObservedRuntimeViews() ([]controllermeta.SlotRuntimeView, bool)
	})
	require.True(t, ok)
	require.Eventually(t, func() bool {
		views, ok := cachedViews.ListCachedObservedRuntimeViews()
		if !ok {
			return false
		}
		for _, view := range views {
			if view.SlotID == targetAssignment.SlotID && view.LeaderID == uint64(expectedLeader) {
				return true
			}
		}
		return false
	}, 10*time.Second, 100*time.Millisecond, "expected follower-applied global runtime view before topology lookup")
